...
from SenderLoss import SenderLoss
from ReceiverLoss import ReceiverLoss
...
class Agents:
    ...
    def __init__(self, vocab, image_embedding_dim, word_embedding_dim,
        ...
        self.sender_loss = SenderLoss()
        self.receiver_loss = ReceiverLoss()
   ...
   # probability distribution for each word
    def get_receiver_selection(self, word, im1_acts, im2_acts):
        ...
        im1_score = torch.sum(im1_mm, dim=1).detach().numpy()[0]

        im2_mm = torch.mul(im2_embed, word_embed)
        im2_score = torch.sum(im2_mm, dim=1).detach().numpy()[0]

        # turns embeddings into probability distribution using the softmax
        # function
        image_probs = torch.softmax(torch.FloatTensor([im1_score, im2_score]), dim=0).detach().numpy()
        # chooses
    ...
    def get_sender_word_probs(self, target_acts, distractor_acts):
        ...
        ordered_embed = np.concatenate([t_embed.detach().numpy(), d_embed.detach().numpy()], axis=0)
        ordered_embed_tensor = torch.from_numpy(ordered_embed)
        # obtains probability distribution for all words from word prob model
        self.word_probs = self.word_probs_model.forward(ordered_embed_tensor).detach().numpy()[0]
    ...
    def update(self, batch):
        ...
        target_acts = np.squeeze(np.array(torch.stack(zip_batch[1]).detach().numpy()))
        distractor_acts = np.squeeze(np.array(torch.stack(zip_batch[2]).detach().numpy()))
        ...
        receiver_update_loss = self.receiver_loss(selected_image_prob, reward)
        sender_update_loss = self.sender_loss(selected_word_prob, reward)
        receiver_update_loss.backward()
        sender_update_loss.backward()

        print("sender_loss : {}".format(sender_update_loss))
        print("receiver_loss : {}".format(receiver_update_loss))
