...
import os
import numpy as np
from agent_detach_np import Agents
import env
import random
import torch
import sys
...
def run_game(config):
    ...
    with torch.enable_grad():
        for i in range(iterations):
            ...
            shuffled_acts = np.concatenate([im1_acts.detach().numpy(), im2_acts.detach().numpy()])
...
def main():
    ...
    # checks if GPU is available to run on
